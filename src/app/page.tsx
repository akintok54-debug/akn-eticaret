...
export default function Home() {
 ...
 <section className="store-hero"><Image src={hero.image||"/akn-workshop.png"} unoptimized alt="Atölyede motosiklet" fill priority sizes="(max-width: 1280px) 100vw, 1280px" className="hero-image"/><div className="hero-shade"/><div className="hero-copy"><div className="eyebrow"><span/> YOLA HAZIR MISIN?</div><h1>{hero.heading||<>Tutkun aynı.<br/>Parçan <em>AKN.</em></>}</h1><p>{hero.description||<>Bakımından performansına, motosikletin için<br className="desktop-break"/> aradığın yedek parça ve aksesuarlar.</>}</p><Link href={hero.href} className="button-red">{hero.buttonText} <span>↗</span></Link><div className="hero-caption">YEDEK PARÇA <i/> BAKIM <i/> AKSESUAR</div></div><span className="hero-index">AKN / MOTO ESSENTIALS</span></section>
 ...
 <PromotionStrip position="categories"/>
 ...
 <PromotionStrip position="products"/>
 ...
 <PromotionStrip position="bottom"/>
 </main><StoreFooter/></>;
}

function PromotionStrip({ position }: { position: "categories" | "products" | "bottom" }) {
 const {promotions}=useStoreDesign();
 const cards=promotions.filter(p=>p.active&&p.position===position);
 if(!cards.length)return null;
 return <section className="store-promotions" aria-label="Reklam ve tanıtımlar">{cards.map(p=>{
  const content=<><Image src={p.image} alt={p.title} width={640} height={360} unoptimized sizes="(max-width: 640px) 100vw, (max-width: 1000px) 50vw, 33vw"/><div><h3>{p.title}</h3>{p.description&&<p>{p.description}</p>}{p.href&&<span>İncele ↗</span>}</div></>;
  return p.href?<Link className="store-promo-card" href={p.href} key={p.id}>{content}</Link>:<article className="store-promo-card" key={p.id}>{content}</article>;
 })}</section>;
}
